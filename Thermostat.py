...
class TemperatureMachine(StateMachine):
    ...
    ## Wrapper methos required by button callbacks and state transitions
        self.updateLights()
    ##
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## Get current temperature safely
            try:
                current_temp = int(floor(self.getFahrenheit()))
            except Exception:
                current_temp = 0

            ## Get set point safely
            try:
            ...
            if altCounter < 6:
                ##
                lcd_line_2 = f"{self.current_state.id} S:{sp} T:{current_temp}"

                altCounter += 1

            else:
                ##
                lcd_line_2 = f"{self.current_state.id.upper():<4} Set:{sp:>3}F"


                altCounter += 1

                ## Every 10 seconds refresh LEDs (keeps indicators in sync)
                if(altCounter >= 11):
            ...
            # Force 16 chars per line for 16x2 LCD
            lcd_line_1 = lcd_line_1.ljust(16)[:16]
